...
const ProfileHeader = () => {
  ...
  return (
    ...
    <View style={[styles.row, { alignItems: 'flex-start' }]}>
      ...
      <TouchableOpacity onPress={onPressChangeProfilePicture} style={{ marginTop: 25 }}>
      ...
      <View>
        <View>
          <Text style={[listStyles.name,{alignSelf: "center", marginLeft:10, marginTop: 0}]}>
            {username}
          </Text>
        </View>

        <TouchableOpacity
            style={[styles.button,{alignSelf: "center",marginLeft: 10. , marginTop: 10}]}
            onPress={logOut}
        >
      ...
      <Text style={[listStyles.name,{marginLeft:25,marginBottom:5, marginTop: 25}]}>My Rides</Text>
    </View>
  );
}
    ...
    const fetchCars = async () => {
      try {
        const response = await fetch(
          `${BASE_URL}/api/users/${username}/rentals`,
        {
          method: 'GET',
        }
      );
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={{ paddingVertical: 35 }}>
            <Text style={{ textAlign: 'center', color: '#666', fontSize: 18, fontWeight: '600' }}>
              Let's Start
            </Text>
          </View>
        }
